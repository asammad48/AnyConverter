# Phase 05 · Day 06: Research Spanish search intent
...
Complete **research spanish search intent**. End the day with **A sourced query-language decision log or an explicit no-access/pending record.** This brief authorizes preparation, research, validation, and the explicitly described later-roadmap work only; it does not authorize inventing results or silently widening scope.
...
- **Inspect exactly:** Authorized GSC query/country exports; external keyword sources when approved; existing keyword reports in `reports/`.
...
3. **Research/decision** — Collect exact source/date/market.
4. **Research/decision** — distinguish impressions from third-party estimates; choose phrasing only above evidence threshold; never invent volume.
...
for p in ['robots.txt','sitemap.xml','reports/']:
