# Phase 05 · Day 03: Audit Spanish canonical signals
...
Complete **audit spanish canonical signals**. End the day with **A URL-level canonical/noindex/status report for every current Spanish page.** This brief authorizes preparation, research, validation, and the explicitly described later-roadmap work only; it does not authorize inventing results or silently widening scope.
...
- **Inspect exactly:** Every `es/**/index.html`; `sitemap.xml`; local server; GSC only if authorized.
...
3. **Research/decision** — Extract canonical and robots directives.
4. **Research/decision** — verify self-canonical intent; compare production response when accessible; keep Google-selected canonical unknown without GSC.
...
for p in ['robots.txt','sitemap.xml','es/**/index.html']:
