# Phase 02 · Day 01: Map the current information architecture
...
Complete **map the current information architecture**. End the day with **A page-to-cluster inventory that preserves existing URLs and flags ambiguous ownership.** This brief authorizes preparation, research, validation, and the explicitly described later-roadmap work only; it does not authorize inventing results or silently widening scope.
...
- **Inspect exactly:** `sitemap.xml`, `index.html`, `pdf-tools/index.html`, `developer-tools/index.html`, `productivity-tools/index.html`.
...
- Read [`../phase-02-architecture-internal-linking.md`](../phase-02-architecture-internal-linking.md) and prerequisites produced by earlier phases and its evidence log before starting.
...
3. **Research/decision** — Parse sitemap and nav links.
4. **Research/decision** — assign pages to primary/secondary clusters; record click paths; flag orphan and multi-cluster candidates.
