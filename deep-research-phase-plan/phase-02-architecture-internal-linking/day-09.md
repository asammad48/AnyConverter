# Phase 02 · Day 09: Create the URL preservation register
...
Complete **create the url preservation register**. End the day with **A list of URLs that must not move and an approval checklist for any exception.** This brief authorizes preparation, research, validation, and the explicitly described later-roadmap work only; it does not authorize inventing results or silently widening scope.
...
- **Inspect exactly:** `sitemap.xml`, GSC exports from Phase 01 when available, `_redirects`.
...
3. **Research/decision** — Record canonical URLs and evidence.
4. **Research/decision** — identify traffic/link unknowns; define migration threshold, redirect mapping, and rollback; default to no move.
