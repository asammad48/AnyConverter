# Phase 02 · Day 10: Specify PDF hub implementation
...
Complete **specify pdf hub implementation**. End the day with **An ordered, file-specific change set for a later coding session.** This brief authorizes preparation, research, validation, and the explicitly described later-roadmap work only; it does not authorize inventing results or silently widening scope.
...
- **Inspect exactly:** `pdf-tools/index.html`; selected PDF tool `index.html` files; shared nav patterns in `index.html`.
...
3. **Research/decision** — Write exact sections, anchors, destinations, breadcrumb labels, and validation steps.
4. **Research/decision** — separate code edits from copy approval.
...
for p in ['robots.txt','sitemap.xml','pdf-tools/index.html']:
