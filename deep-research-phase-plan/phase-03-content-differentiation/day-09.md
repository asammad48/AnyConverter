# Phase 03 · Day 09: Brief JSON and CSV tools
...
Complete **brief json and csv tools**. End the day with **Implementation-ready briefs for JSON Formatter, CSV to SQL, and CSV Query.** This brief authorizes preparation, research, validation, and the explicitly described later-roadmap work only; it does not authorize inventing results or silently widening scope.
...
- **Inspect exactly:** `json-formatter/index.html`, `csv-to-sql/index.html`, `csv-query/index.html`.
...
- Read [`day-08.md`](day-08.md) and its evidence log before starting.
...
3. **Research/decision** — Test malformed/valid samples.
4. **Research/decision** — document dialect/encoding/size behavior only when observed; create input-output examples and next links.
...
for p in ['robots.txt','sitemap.xml','json-formatter/index.html']:
...
- A concise handoff for [`day-10.md`](day-10.md).
...
Pass to [`day-10.md`](day-10.md): the decision summary, evidence-log path, raw artifact references, exact known/unknown list, unresolved blockers with owners, files/URLs/selectors inspected, and the first safe next command. State explicitly whether downstream work is **unblocked**, **partially unblocked**, or **blocked**.

## Evidence contract

| Contract field | Requirement for Day 09 |
|---|---|
