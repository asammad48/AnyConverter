# Phase 03 · Day 01: Inventory repeated content patterns

**Phase timeline preserved from the source report:** Weeks 3-4. **Planning unit:** one focused workday.

## Objective and end-of-day outcome

Complete **inventory repeated content patterns**. End the day with **A quoted-and-located boilerplate inventory across priority PDF and developer pages.** This brief authorizes preparation, research, validation, and the explicitly described later-roadmap work only; it does not authorize inventing results or silently widening scope.
...
- **Source of truth:** [`../phase-03-content-differentiation.md`](../phase-03-content-differentiation.md).
...
- **Inspect exactly:** Top PDF/developer candidates in Phase 03 overview; `reports/all-page-keyword-recommendations-2026-08-21.csv`.
- **External surfaces, only when access is authorized:** Rendered local pages, browser Network/console output, and authorized GSC data.
...
- Read [`../phase-03-content-differentiation.md`](../phase-03-content-differentiation.md) and prerequisites produced by earlier phases and its evidence log before starting.
- Phase 03 consumes the accepted outputs named in the root [dependency matrix](../README.md#cross-phase-dependency-matrix). If any required milestone is missing, continue only with work that does not depend on it.
...
1. What does the implementation demonstrably do, limit, store, or transmit?
2. Which copy is interchangeable boilerplate rather than tool-specific help?
3. What example and failure guidance can be reproduced with synthetic data?
...
3. **Research/decision** — Sample all candidates.
4. **Research/decision** — hash or compare repeated sections; record selectors/headings and line locations; separate shared UI from weak body copy.
...
for p in ['robots.txt','sitemap.xml','reports/all-page-keyword-recommendations-2026-08-21.csv']:
...
- A concise handoff for [`day-02.md`](day-02.md).
...
Pass to [`day-02.md`](day-02.md): the decision summary, evidence-log path, raw artifact references, exact known/unknown list, unresolved blockers with owners, files/URLs/selectors inspected, and the first safe next command. State explicitly whether downstream work is **unblocked**, **partially unblocked**, or **blocked**.

## Evidence contract

| Contract field | Requirement for Day 01 |
|---|---|
