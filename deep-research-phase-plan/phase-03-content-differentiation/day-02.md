# Phase 03 · Day 02: Create the accuracy fact sheet
...
Complete **create the accuracy fact sheet**. End the day with **A per-tool fact matrix for formats, limits, processing location, network behavior, and known errors.** This brief authorizes preparation, research, validation, and the explicitly described later-roadmap work only; it does not authorize inventing results or silently widening scope.
...
- **Inspect exactly:** Priority tool HTML/JavaScript; `privacy/index.html`; `security/index.html`; browser Network panel.
...
3. **Research/decision** — Trace actual implementation.
4. **Research/decision** — test representative inputs; record only observed limits and requests; route uncertain claims to engineering.
...
for p in ['robots.txt','sitemap.xml','privacy/index.html']:
