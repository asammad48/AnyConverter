# Phase 03 · Day 11: Brief hash and cron tools
...
Complete **brief hash and cron tools**. End the day with **Implementation-ready briefs for Hash Generator and Cron Expression Parser.** This brief authorizes preparation, research, validation, and the explicitly described later-roadmap work only; it does not authorize inventing results or silently widening scope.
...
- **Inspect exactly:** `hash-generator/index.html`, `cron-expression-parser/index.html`.
...
- Read [`day-10.md`](day-10.md) and its evidence log before starting.
...
3. **Research/decision** — Verify algorithms/dialects/timezone behavior from code and tests.
4. **Research/decision** — use synthetic examples; document unsupported semantics as unknown.
...
for p in ['robots.txt','sitemap.xml','hash-generator/index.html']:
...
- A concise handoff for [`day-12.md`](day-12.md).
...
Pass to [`day-12.md`](day-12.md): the decision summary, evidence-log path, raw artifact references, exact known/unknown list, unresolved blockers with owners, files/URLs/selectors inspected, and the first safe next command. State explicitly whether downstream work is **unblocked**, **partially unblocked**, or **blocked**.

## Evidence contract

| Contract field | Requirement for Day 11 |
|---|---|
