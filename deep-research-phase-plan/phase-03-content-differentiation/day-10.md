# Phase 03 · Day 10: Brief decoding and pattern tools
...
Complete **brief decoding and pattern tools**. End the day with **Implementation-ready briefs for JWT Decoder, Regex Tester, Base64 Encoder, and URL Encoder.** This brief authorizes preparation, research, validation, and the explicitly described later-roadmap work only; it does not authorize inventing results or silently widening scope.
...
- **Inspect exactly:** `jwt-decoder/index.html`, `regex-tester/index.html`, `base64-encoder/index.html`, `url-encoder/index.html`.
...
3. **Research/decision** — Test Unicode and malformed inputs.
4. **Research/decision** — state decoder-not-verifier distinctions if implementation supports only decode; never use real secrets.
...
for p in ['robots.txt','sitemap.xml','jwt-decoder/index.html']:
