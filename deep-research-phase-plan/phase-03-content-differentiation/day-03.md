# Phase 03 · Day 03: Prioritize the first 20 rewrites
...
Complete **prioritize the first 20 rewrites**. End the day with **An evidence-ranked queue of up to ten PDF and ten developer pages.** This brief authorizes preparation, research, validation, and the explicitly described later-roadmap work only; it does not authorize inventing results or silently widening scope.
...
- **Inspect exactly:** Phase 01 GSC baseline if available; Day 01 boilerplate inventory; Day 02 fact sheet.
...
- Read [`day-02.md`](day-02.md) and its evidence log before starting.
...
3. **Research/decision** — Score business fit, impressions, quality gap, and safety.
4. **Research/decision** — use product priority when GSC unavailable; document why excluded pages wait.
...
for p in ['robots.txt','sitemap.xml','index.html']:
...
- A concise handoff for [`day-04.md`](day-04.md).
...
Pass to [`day-04.md`](day-04.md): the decision summary, evidence-log path, raw artifact references, exact known/unknown list, unresolved blockers with owners, files/URLs/selectors inspected, and the first safe next command. State explicitly whether downstream work is **unblocked**, **partially unblocked**, or **blocked**.

## Evidence contract

| Contract field | Requirement for Day 03 |
|---|---|
