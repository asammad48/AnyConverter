# Phase 04 · Day 13: Dry-run the gates without roadmap changes
...
Complete **dry-run the gates without roadmap changes**. End the day with **A dry-run report proving the checks execute and distinguishing existing failures from gate defects.** This brief authorizes preparation, research, validation, and the explicitly described later-roadmap work only; it does not authorize inventing results or silently widening scope.
...
- **Inspect exactly:** All gate specifications; local server; representative pages.
...
3. **Research/decision** — Run local checks.
4. **Research/decision** — manually exercise fixtures; log existing issues rather than fixing roadmap scope; refine commands with false-positive evidence.
