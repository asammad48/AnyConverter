# Phase 04 · Day 02: Capture repository-local performance baseline
...
Complete **capture repository-local performance baseline**. End the day with **A local baseline for the seven priority URLs, clearly not represented as production field performance.** This brief authorizes preparation, research, validation, and the explicitly described later-roadmap work only; it does not authorize inventing results or silently widening scope.
...
- **Inspect exactly:** Homepage; `/pdf-merge/`; `/pdf-compress/`; `/image-converter/`; `/json-formatter/`; `/word-counter/`; `/typing-speed-test/`.
...
3. **Research/decision** — Build/serve consistently.
4. **Research/decision** — run repeated Lighthouse checks; retain reports; calculate medians; label environment and limitations.
