# Phase 04 · Day 07: Specify lazy-loading and code-splitting work
...
Complete **specify lazy-loading and code-splitting work**. End the day with **A route-specific implementation plan with expected measurement and rollback, not speculative savings.** This brief authorizes preparation, research, validation, and the explicitly described later-roadmap work only; it does not authorize inventing results or silently widening scope.
...
- **Inspect exactly:** Day 04 loading matrix; affected HTML/script references; `PERFORMANCE.md`.
...
3. **Research/decision** — Rank candidates by measured cost.
4. **Research/decision** — specify dynamic-load trigger and fallback; identify regression tests; avoid global refactors without ownership.
