# Phase 04 · Day 08: Define the SEO release gate
...
Complete **define the seo release gate**. End the day with **An automatable gate for build, status, robots, sitemap, canonical, noindex, hreflang, and links.** This brief authorizes preparation, research, validation, and the explicitly described later-roadmap work only; it does not authorize inventing results or silently widening scope.
...
- **Inspect exactly:** `robots.txt`, `sitemap.xml`, `_redirects`, `_headers`, `SEO_SPEC.md`, `reports/seo-deployment-checklist-2026-08-21.md`.
...
3. **Research/decision** — Map each check to command, expected result, and severity.
4. **Research/decision** — distinguish local/deployed checks; define stop-ship thresholds.
