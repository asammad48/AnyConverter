# Phase 04 · Day 01: Define measurement protocol

**Phase timeline preserved from the source report:** Weeks 4-5, then every release. **Planning unit:** one focused workday.

## Objective and end-of-day outcome

Complete **define measurement protocol**. End the day with **A repeatable lab/field measurement protocol with device, network, runs, URLs, and evidence rules.** This brief authorizes preparation, research, validation, and the explicitly described later-roadmap work only; it does not authorize inventing results or silently widening scope.
...
- **Source of truth:** [`../phase-04-performance-privacy-release-gates.md`](../phase-04-performance-privacy-release-gates.md).
...
- **Inspect exactly:** `PERFORMANCE.md`; Phase 04 budget table; Lighthouse/PageSpeed documentation.
- **External surfaces, only when access is authorized:** PageSpeed Insights, GSC Core Web Vitals, deployed pages, and browser DevTools.
...
- Read [`../phase-04-performance-privacy-release-gates.md`](../phase-04-performance-privacy-release-gates.md) and prerequisites produced by earlier phases and its evidence log before starting.
- Phase 04 consumes the accepted outputs named in the root [dependency matrix](../README.md#cross-phase-dependency-matrix). If any required milestone is missing, continue only with work that does not depend on it.
...
1. Is evidence lab, field, local, or deployed, and are comparisons compatible?
2. Which request or dependency is necessary for the current route?
3. What objective regression must stop a release?
...
3. **Research/decision** — Choose fixed Lighthouse settings and median-run policy.
4. **Research/decision** — distinguish lab from CrUX/GSC field data; define artifact names; mark unavailable field data unknown.
...
for p in ['robots.txt','sitemap.xml','PERFORMANCE.md']:
