# Phase 04 · Day 05: Audit third-party requests
...
Complete **audit third-party requests**. End the day with **A request inventory with purpose, initiator, data class, consent requirement, and owner.** This brief authorizes preparation, research, validation, and the explicitly described later-roadmap work only; it does not authorize inventing results or silently widening scope.
...
- **Inspect exactly:** All priority pages; browser Network panel; `_headers`; `privacy/index.html`.
...
3. **Research/decision** — Clear cache and capture page-load/tool-action traffic.
4. **Research/decision** — classify first/third party; inspect payload names without storing user content; escalate unexplained calls.
...
for p in ['robots.txt','sitemap.xml','_headers']:
