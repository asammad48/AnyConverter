# Phase 01 · Day 05: Prepare Search Console verification

**Phase timeline preserved from the source report:** Week 1. **Planning unit:** one focused workday.

## Objective and end-of-day outcome

Complete **prepare search console verification**. End the day with **A GSC access/runbook and evidence-log rows for property ownership, sitemap submission, and coverage reports.** This brief authorizes preparation, research, validation, and the explicitly described later-roadmap work only; it does not authorize inventing results or silently widening scope.
...
- **Source of truth:** [`../phase-01-foundation-baseline.md`](../phase-01-foundation-baseline.md).
...
- **Inspect exactly:** `reports/google-search-console-audit-2026-08-20.md`, `reports/search-console-followup-2026-08-26.md`, `https://search.google.com/search-console`.
- **External surfaces, only when access is authorized:** Google Search Console and the deployed `https://anyconverter.io/` endpoints when authorized.
...
- Read [`day-04.md`](day-04.md) and its evidence log before starting.
- Phase 01 consumes the accepted outputs named in the root [dependency matrix](../README.md#cross-phase-dependency-matrix). If any required milestone is missing, continue only with work that does not depend on it.
...
1. What can be proven from repository state, and what requires deployed or GSC evidence?
2. Which observed issue blocks crawling, trust, or measurement?
3. Does the sample justify only a sample finding, or a site-wide conclusion?
...
3. **Research/decision** — Review historical reports.
4. **Research/decision** — define property and date windows; list exports/screenshots required; perform console checks only with authorized access; mark unavailable facts unknown.
...
for p in ['robots.txt','sitemap.xml','reports/google-search-console-audit-2026-08-20.md']:
...
- A concise handoff for [`day-06.md`](day-06.md).
...
Pass to [`day-06.md`](day-06.md): the decision summary, evidence-log path, raw artifact references, exact known/unknown list, unresolved blockers with owners, files/URLs/selectors inspected, and the first safe next command. State explicitly whether downstream work is **unblocked**, **partially unblocked**, or **blocked**.

## Evidence contract

| Contract field | Requirement for Day 05 |
|---|---|
