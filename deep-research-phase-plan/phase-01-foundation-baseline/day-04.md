...
- **Inspect exactly:** `_redirects`, `wrangler.jsonc`, `_headers`, `https://anyconverter.io/`.
...
3. **Research/decision** — Review redirect configuration.
4. **Research/decision** — test four hostname variants with curl when permitted; count hops; distinguish repository intent from deployed behavior.
...
for p in ['robots.txt','sitemap.xml','_redirects']:
