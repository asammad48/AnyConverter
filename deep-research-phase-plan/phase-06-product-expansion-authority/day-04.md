# Phase 06 · Day 04: Research HAR analyzer
...
Complete **research har analyzer**. End the day with **An evidence pack and score for a client-side HAR analyzer with an explicit sensitive-data threat model.** This brief authorizes preparation, research, validation, and the explicitly described later-roadmap work only; it does not authorize inventing results or silently widening scope.
...
- **Inspect exactly:** HAR specification/browser export docs; `developer-tools/index.html`; existing parser patterns.
...
- Read [`day-03.md`](day-03.md) and its evidence log before starting.
...
3. **Research/decision** — Define jobs and safe fixture.
4. **Research/decision** — assess parsing feasibility and redaction risks; evaluate cluster/linkability; block if safe handling cannot be specified.
...
for p in ['robots.txt','sitemap.xml','developer-tools/index.html']:
...
- A concise handoff for [`day-05.md`](day-05.md).
...
Pass to [`day-05.md`](day-05.md): the decision summary, evidence-log path, raw artifact references, exact known/unknown list, unresolved blockers with owners, files/URLs/selectors inspected, and the first safe next command. State explicitly whether downstream work is **unblocked**, **partially unblocked**, or **blocked**.

## Evidence contract

| Contract field | Requirement for Day 04 |
|---|---|
