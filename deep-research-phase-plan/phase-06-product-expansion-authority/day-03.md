# Phase 06 · Day 03: Research EXIF and PDF metadata tools
...
Complete **research exif and pdf metadata tools**. End the day with **An evidence pack and score for EXIF viewer/remover and PDF metadata remover.** This brief authorizes preparation, research, validation, and the explicitly described later-roadmap work only; it does not authorize inventing results or silently widening scope.
...
- **Inspect exactly:** Candidate SERPs/official format docs when approved; existing image/PDF code; `image-converter/index.html`, `pdf-security/index.html`.
...
3. **Research/decision** — Validate intent evidence.
4. **Research/decision** — assess browser APIs/libraries and metadata removal limits; identify privacy differentiation; record competition without fabricated metrics.
...
for p in ['robots.txt','sitemap.xml','image-converter/index.html']:
