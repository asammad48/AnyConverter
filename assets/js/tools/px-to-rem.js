(function() {
  ...
  function buildTable() {
    ...
    var html = '<table style="width:100%;border-collapse:collapse;font-size:13px"><thead><tr>' +
      '<th style="padding:8px 12px;text-align:left;border-bottom:2px solid var(--color-border,#DDD8D0);background:var(--color-surface-2,#F5F1EE)">px</th>' +
      '<th style="padding:8px 12px;text-align:left;border-bottom:2px solid var(--color-border,#DDD8D0);background:var(--color-surface-2,#F5F1EE)">rem</th>' +
      '<th style="padding:8px 12px;text-align:left;border-bottom:2px solid var(--color-border,#DDD8D0);background:var(--color-surface-2,#F5F1EE)">Common Use</th></tr></thead><tbody>';
    ...
    common.forEach(function(px) {
      html += '<tr><td style="padding:7px 12px;border-bottom:1px solid var(--color-border-light,#ECE7DF)">' + px + '</td>' +
        '<td style="padding:7px 12px;border-bottom:1px solid var(--color-border-light,#ECE7DF);font-family:monospace">' + (px/base).toFixed(4).replace(/\.?0+$/,'') + '</td>' +
        '<td style="padding:7px 12px;border-bottom:1px solid var(--color-border-light,#ECE7DF);color:var(--color-text-2,#3F3A36)">' + (uses[px]||'') + '</td></tr>';
    });
    html += '</tbody></table>';
